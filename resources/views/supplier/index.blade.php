            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        <!-- Grid column -->
                        <div class="col-md-12">
                            <h2 class="py-3 text-center font-bold font-up blue-text">Table Supplier</h2>
                        </div>
                        <!-- Grid column -->
                    </div>
                    <!-- Grid row -->
                    ...
                    <div class="rae">
                    <a href="{{route('add-supplier')}}" class="btn btn-success btn-sm">+Add</a>
                    </div>
                    <!--Table-->
                    <table class="table table-hover table-responsive mb-0">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="th-lg"><a>Nama Supplier</th>
                        ...
                        <!--Table body-->
                        <tbody>
                            @foreach ($master_supplier as $data)
                            <tr>
                                    <th scope="row">{{$data->kode_supplier}}</th>
                                    <td>{{$data->nama_supplier}}</td>
                                    <td>{{$data->no_telp_supplier}}</td>
                                    <td>{{$data->alamat_supplier}}</td>

                                    <td>
                                        <a href="{{route('delete-supplier', $data->kode_supplier)}}" class="btn btn-danger btn-sm">Hapus</a>
                                        <a href="{{route('show-supplier', $data->kode_supplier)}}" class="btn btn-primary btn-sm">Update</a>
                                    </td>
                    ...
                    {{ $master_supplier->render("pagination::bootstrap-4") }}
                    <!--Bottom Table UI-->
                </div>
            </div>
